...
export default function JobPaper({ decorationFinished }) {
    ...
    const [message, setMessage] = useState({ visible: false, e: null });
                    ...
                    {apply2 ?
                        ...
                        secondSlide && apply ?
                            <SecondSlide
                                ...
                                starWasClicked={starWasClicked}
                                setApply2={setApply2}
                                setMessage={setMessage}
                            /> :
                            apply ?
                                <Spinner setSecondSlide={setSecondSlide} /> :
                                <FirstSlide
                                    handleStarClick={handleStarClick}
                                    starWasClicked={starWasClicked}
                                    setApply={setApply}
...
function FirstSlide({ handleStarClick, starWasClicked, setApply, setMessage }) {
    return (
        <>
            <TopPaperOptions
                handleStarClick={handleStarClick}
            ...
            <section className={styles.content}>
                ...
                <button className={styles.apply} onClick={() => setApply(true)} >APPLY NOW</button>
...
function SecondSlide({ handleStarClick, starWasClicked, setApply2, setMessage }) {
    const [showTasks, setShowTasks] = useState(false);
    const [completed, setCompleted] = useState({ one: false, two: false });
    ...
    function handleCheckboxes(e) {
        if (e.target.id === 'instagram') {
            setCompleted(prevState => ({ ...prevState, one: !prevState.one }))
        } else {
            setCompleted(prevState => ({ ...prevState, two: !prevState.two }))
        }
    }
    ...
    return (
        <div className={styles.secondSlide}>
            <TopPaperOptions
                ...
                setMessage={setMessage}
            />
            <div className={showTasks ? styles.title + ' ' + styles.lessMargin : styles.title}>
                <h1>Complete Tasks</h1>
                <p>To continue, complete the tasks below</p>
                <div className={styles.line}></div>
            </div>
            ...
            {showTasks ?
                    ...
                    <div className={styles.tasks}>
                        <ul>
                            <li>
                                <div>
                                    <div>
                                        <span className={styles.bolder}>Task 1:&nbsp;&nbsp;</span><span>Unblock @armando.koleci on Instagram</span>
                                    </div>
                                    <Checkbox handleChange={handleCheckboxes} id={"instagram"} />
                                </div>
                            </li>
                        </ul>
                        <ul>
                            <li>
                                <div>
                                    <div>
                                        <span className={styles.bolder}>Task 2:&nbsp;&nbsp;</span><span>Unblock +420 604 886 244 on WhatsApp</span>
                                    </div>
                                    <Checkbox handleChange={handleCheckboxes} id={"whatsapp"} />
                                </div>
                            </li>
                        </ul>
                    </div>
                    <button
                        className={completed.one && completed.two ?
                            styles.apply :
                ...
                <div className={styles.chevron}>
                    <div onClick={handleClick} className={styles.chevronContainer}>
                        <Button>
                            <img src="/chevron.png" />
                        </Button>
                    </div>
...
function ThirdSlide() {
    return (
        <div className={styles.title + ' ' + styles.thirdSlide}>
            <h1>Sorry there is no job!</h1>
...
function TopPaperOptions({ handleStarClick, starWasClicked, setMessage }) {
    return (
        <div className={styles.top}>
            <img src="/share.png" onClick={() => setMessage({ visible: true, e: "share", hook: setMessage })} />
            <img
            ...
            />
            <div className={styles.moreJobs} onClick={() => setMessage({ visible: true, e: null, hook: setMessage })}>
                <img src="/arrow.png" />
                <p>View More Jobs</p>
            </div>
        </div>
